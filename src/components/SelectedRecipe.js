...
export default function SelectedRecipe() {
    ...
    const getSelectedRecipe=async()=>{
        let rawData=await fetch(`https://recipe-app-backend-api.herokuapp.com/recipe/${id}`,{
            method:"GET"
        })
  ...
  return (
    <>
    <Button variant="contained" color="primary" style={{marginRight:'auto'}} onClick={()=>{history.push("/recipes")}}>{`<--Back To All Recipes`}</Button>
    <br/>
    <Card className={classes.root}>
      ...
      />
      <hr/>
      <RecipeContent imageUrl={currentRecipe.url} description={currentRecipe.description} />
      ...
      <CardActions disableSpacing>
        ...
        >
          <ExpandMoreIcon />{!expanded?<span style={{fontSize:"0.5em"}}>Show procedure</span>:""}
        </IconButton>
      </CardActions>
      <hr/>
